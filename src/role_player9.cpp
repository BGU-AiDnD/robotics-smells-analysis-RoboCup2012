...
int RolePlayer9::S_count_shot_area_hold = 0;

#define USE_BHV_OFFENSIVE_HALF_DEFENSIVE_MOVE
#define USE_BHV_OFFENSIVE_HALF_OFFENSIVE_MOVE

rcsc::Vector2D RolePlayer9::p9AP;

rcsc::Vector2D RolePlayer9::p9DP;

rcsc::Vector2D RolePlayer9::p9VAP;

rcsc::Vector2D RolePlayer9::p9VDP;

rcsc::Vector2D RolePlayer9::p9PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9PB5 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9PB6 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9PB7 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9PB8 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9PB10 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer9::p9LB = rcsc::Vector2D(-70.0,-70.0);

//swarm parameters

#define NS 6        //size of neighborhood
#define c1 1.44
#define c2 0.8
...
*/
void
RolePlayer9::execute( rcsc::PlayerAgent * agent )
{
    //////////////////////////////////////////////////////////////
    // play_on play
...
*/
void
RolePlayer9::doKick( rcsc::PlayerAgent * agent )
{
...
/*      if( kickCentering( agent ) )
	...
	break;
      ...
      if(Bhv_GEARShoot().execute(agent))
      	break;	
...
*/
void
RolePlayer9::doMove( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p9AP;
        ...
        {
            rcsc::Vector2D target_point = p9DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            //rcsc::Body_GoToPoint(target_point, dist_thr, dash_power, true);
            //rcsc::Body_GoToPointDodge(target_point, dash_power);
...
*/
void
RolePlayer9::doDribbleAttackKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer9::doShootChanceKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer9::doOffensiveMove( rcsc::PlayerAgent * agent,
...
*/
void
RolePlayer9::doDefensiveMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
void
RolePlayer9::doShootAreaMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
bool
RolePlayer9::doShootAreaIntercept( rcsc::PlayerAgent * agent,
                                         const rcsc::Vector2D & /*home_pos*/ )
{
    const rcsc::WorldModel & wm = agent->world();
    const int self_min = wm.interceptTable()->selfReachCycle();
...
int
RolePlayer9::countKickableCycleAtSamePoint(rcsc::PlayerAgent * agent){
...
*/
void
RolePlayer9::doShootAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
bool
RolePlayer9::doCheckCrossPoint( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
void RolePlayer9::doOffensiveMiddleKick( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer9::doCrossAreaKick( rcsc::PlayerAgent* agent )
{
...
*/
void
RolePlayer9::doDribbleAttackAreaKick( rcsc::PlayerAgent* agent )
{
    // !!!check stamina!!!
    static bool S_recover_mode = false;
...
*/
bool
RolePlayer9::doStraightDribble( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer9::doSideForwardDribble( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
rcsc::Vector2D
RolePlayer9::getDribbleTarget( rcsc::PlayerAgent * agent )
{
...
*/
int
RolePlayer9::getDribbleAttackDashStep( const rcsc::PlayerAgent * agent,
...
bool
RolePlayer9::kickCentering( rcsc::PlayerAgent * agent )
{
  //センタリング 090704
  const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer9::doMiddleAreaKick( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer9::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer9::swarmUpdatePosition(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
...
void
RolePlayer9::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(5) != 0) p9PB5 = wm.teammate(5)->pos();

        if(wm.teammate(6) != 0) p9PB6 = wm.teammate(6)->pos();

        if(wm.teammate(7) != 0) p9PB7 = wm.teammate(7)->pos();

        if(wm.teammate(8) != 0) p9PB8 = wm.teammate(8)->pos();

        if(wm.teammate(9) != 0) p9PB = wm.teammate(9)->pos();

        if(wm.teammate(10) != 0) p9PB10 = wm.teammate(10)->pos();

        if(wm.teammate(10) != 0) p9LB = wm.teammate(10)->pos();

        p9VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p9VDP.x = wm.ball().pos().x - wm.self().pos().x;

        p9AP = rcsc::Vector2D(-50.0,0.0);
        p9DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        {
            //X
            if(wm.teammate(i) != 0 && (wm.teammate(i)->pos().x > PB[i-5].x) )
                PB[i-5] = wm.teammate(i)->pos();
        }

        //only updates variable if new position is better and valid

        if(p9PB5 != PB[0]) p9PB5 = PB[0];

        if (p9PB6 != PB[1]) p9PB6 = PB[1];

        if (p9PB7 != PB[2]) p9PB7 = PB[2];

        if (p9PB8 != PB[3]) p9PB8 = PB[3];

        if (p9PB != PB[4]) p9PB = PB[4];

        if (p9PB10 != PB[5]) p9PB10 = PB[5];
        ...
        //speed.x = 0.2*p9VAP.x;
        speed.x += c1 * k * (PB[4].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        speed.y = 0.0;

        p9AP = wm.self().pos() + speed;
        p9DP = wm.self().pos() + speed;

        p9VAP = p9AP;       //represents inertia
        p9VDP = p9DP;
    }
}
...
void
RolePlayer9::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
