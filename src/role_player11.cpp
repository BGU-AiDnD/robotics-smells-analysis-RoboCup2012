...
int RolePlayer11::S_count_shot_area_hold = 0;

rcsc::Vector2D RolePlayer11::p11AP;

rcsc::Vector2D RolePlayer11::p11DP;

rcsc::Vector2D RolePlayer11::p11VAP;

rcsc::Vector2D RolePlayer11::p11VDP;

rcsc::Vector2D RolePlayer11::p11PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer11::p11PB9 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer11::p11PB10 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer11::p11LB = rcsc::Vector2D(-50.0,-50.0);
...
*/
void
RolePlayer11::execute( rcsc::PlayerAgent * agent )
{
...
void
RolePlayer11::doKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer11::doMove( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p11AP;
        ...
        {
            rcsc::Vector2D target_point = p11DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            //rcsc::Body_GoToPoint(target_point, dist_thr, dash_power, true);
            //rcsc::Body_GoToPointDodge(target_point, dash_power);
...
int
RolePlayer11::countKickableCycleAtSamePoint(rcsc::PlayerAgent * agent){
...
bool
RolePlayer11::moveCentering( rcsc::PlayerAgent * agent )
{
  const rcsc::WorldModel & wm = agent->world();
...
*/
bool
RolePlayer11::doSelfPass( rcsc::PlayerAgent  * agent,
...
*/
void
RolePlayer11::doMiddleAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer11::doCrossAreaKick( rcsc::PlayerAgent* agent )
{
...
*/
void
RolePlayer11::doShootAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer11::doGoToCrossPoint( rcsc::PlayerAgent * agent,
...
*/
bool
RolePlayer11::doCheckCrossPoint( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
void RolePlayer11::doOffensiveMiddleKick( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer11::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer11::swarmUpdatePosition(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
...
void
RolePlayer11::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(9) != 0) p11PB9 = wm.teammate(9)->pos();

        if(wm.teammate(10) != 0) p11PB10 = wm.teammate(10)->pos();

        if(wm.teammate(11) != 0) p11PB = wm.teammate(11)->pos();

        if(wm.teammate(11) != 0) p11LB = wm.teammate(11)->pos();

        p11VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p11VDP.x = wm.ball().pos().x - wm.self().pos().x;

        p11AP = rcsc::Vector2D(-50.0,0.0);
        p11DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        if(p11PB9 != PB[0]) p11PB9 = PB[0];

        if (p11PB10 != PB[1]) p11PB10 = PB[1];

        if (p11PB != PB[2]) p11PB = PB[2];
        ...
        //speed.x = 0.2*p11VAP.x;
        speed.x += c1 * k * (PB[2].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        speed.y = 0.0;

        p11AP = wm.self().pos() + speed;
        p11DP = wm.self().pos() + speed;

        p11VAP = p11AP;       //represents inertia
        p11VDP = p11DP;
    }
}
...
void
RolePlayer11::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
