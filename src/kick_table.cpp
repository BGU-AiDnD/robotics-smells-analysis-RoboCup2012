...
const double KickTable::FAR_SIDE_RATE = 0.68;
...
KickTable::calc_far_dist( const PlayerType & player_type )
{
    ...
    //                + ServerParam::i().ballSize() ),
    //              player_type.kickableArea() - 0.225 );
...
{
    const double ball_speed_max2 = std::pow( ServerParam::i().ballSpeedMax(), 2.0 );
...
KickTable::createStateCache( const WorldModel & world )
{
    ...
    const ServerParam & param = ServerParam::i();
    const Rect2D pitch( Vector2D( - param.pitchHalfLength(),
                                       ...
                                       const double & first_speed )
{
    ...
    const double collide_dist2 = std::pow( self_type.playerSize() + ServerParam::i().ballSize(), 2.0 );
                             ...
                             State & state )
{
          ...
          ++o )
    {
        ...
        const double max_accel = ( ServerParam::i().maxPower()
                                       ...
                                       State & state )
{
          ...
          ++o )
    {
        ...
        double control_area = ( ( (*o)->goalie()
                                ...
                                : (*o)->playerTypePtr()->kickableArea() );
        control_area += 0.15;
        double control_area2 = control_area * control_area;
    ...
    for ( int i = 0; i < NUM_STATE; ++i )
    {
...
#if 1
        {
            ...
            if ( ( my_noise + ball_noise + max_kick_rand ) * 0.9
                 > my_kickable_area - state.dist_ - 0.1 )
            {
#ifdef DEBUG
                dlog.addText( Logger::KICK,
          ...
          ++it, ++count )
    {
...
#if 1
        {
            ...
            double max_kick_rand
                ...
                * ( kick_power / param.maxPower() )
                * ( current_pos_rate + current_speed_rate );
            if ( ( my_noise1 + ball_noise + max_kick_rand ) * 0.9
                 > my_kickable_area - state_1st.dist_ - 0.1 )
            {
#ifdef DEBUG
                dlog.addText( Logger::KICK,
                              "xx__ 3 step: failed. buffer is not safety. power=%.3f"
                              " my_kickable=%.3f state_dist=%.3f,"
                              " noise=%f(my_noise=%f ball_noise=%f kick_rand=%f)",
                              kick_power,
