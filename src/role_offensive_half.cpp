...
*/
void
RoleOffensiveHalf::doDribbleAttackKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RoleOffensiveHalf::doShootChanceKick( rcsc::PlayerAgent * agent )
{
                                    ...
                                    const rcsc::Vector2D & home_pos )
{
         ...
         && wm.ball().pos().dist( home_pos ) < 10.0 )
    {
        ...
        if ( attack_point.x < home_pos.x + 5.0 )
        {
            ...
            rcsc::Body_GoToPoint( attack_point, 0.5, rcsc::ServerParam::i().maxPower() ).execute( agent );
            agent->setNeckAction( new rcsc::Neck_TurnToBallOrScan() );
            return;
        }
    }
...
*/
void
RoleOffensiveHalf::doShootAreaMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
    ...
    agent->debugClient().addMessage( "ShootMove%.0f", rcsc::ServerParam::i().maxPower() );
    agent->debugClient().setTarget( target_point );
    agent->debugClient().addCircle( target_point, dist_thr );
    rcsc::dlog.addText( rcsc::Logger::ROLE,
                        __FILE__": doShootAreaMove() chance area move to (%.1f, %.1f) power=%.1f",
                        target_point.x, target_point.y, rcsc::ServerParam::i().maxPower() );

    if ( ! rcsc::Body_GoToPoint( target_point, dist_thr, rcsc::ServerParam::i().maxPower() ).execute( agent ) )
    {
...
*/
bool
RoleOffensiveHalf::doShootAreaIntercept( rcsc::PlayerAgent * agent,
                                         const rcsc::Vector2D & /*home_pos*/ )
{
    const rcsc::WorldModel & wm = agent->world();
    const int self_min = wm.interceptTable()->selfReachCycle();
...
void
RoleOffensiveHalf::doMiddleAreaKick( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RoleOffensiveHalf::doDribbleBlockMove( rcsc::PlayerAgent* agent,
                                       const rcsc::Vector2D& home_pos )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
    ...
    else
    {
        ...
        dash_power = rcsc::min_max( 0.0, rcsc::ServerParam::i().maxPower(), dash_power );
        rcsc::dlog.addText( rcsc::Logger::ROLE,
                            __FILE__": doDribbleBlockMove() dash_power=%.1f",
                            dash_power );
    }
    ...
    dash_power = wm.self().getSafetyDashPower( rcsc::ServerParam::i().maxPower() );
    ...
    if ( ! rcsc::Body_GoToPoint( home_pos, dist_thr, rcsc::ServerParam::i().maxPower() ).execute( agent ) )
    {
        // face to front or side
        rcsc::AngleDeg body_angle = 0.0;
...
*/
void
RoleOffensiveHalf::doCrossBlockMove( rcsc::PlayerAgent* agent,
                                     const rcsc::Vector2D& home_pos )
{
