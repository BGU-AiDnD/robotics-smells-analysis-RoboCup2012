...
*/
void
RoleSweeper::doMove( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    rcsc::Vector2D home_pos = wm.self().pos();
    ...
    case Strategy::BA_ShootChance:
        ...
        //if ball is close, do defensive move

        if(wm.self().distFromBall() < 10){
            Bhv_CenterBackDangerMove().execute( agent );
            agent->setNeckAction( new rcsc::Neck_TurnToBall() );
            break;
        }

        //if ball is ahead of center backs
        
        else if(wm.ball().pos().x >= wm.teammate(2)->pos().x ||
                wm.ball().pos().x >= wm.teammate(3)->pos().x){
            target_point.x = ((wm.teammate(2)->pos().x + wm.teammate(3)->pos().x )/2) - 5;
            target_point.y = wm.ball().pos().y;
            agent->setNeckAction( new rcsc::Neck_TurnToBall() );
        ...
        //if ball is between center backs and sweeper

        else if(wm.ball().pos().x <= wm.teammate(2)->pos().x ||
                wm.ball().pos().x <= wm.teammate(3)->pos().x){
            target_point.x = (wm.ball().pos().x) - 2;
            target_point.y = wm.ball().pos().y;
            agent->setNeckAction( new rcsc::Neck_TurnToBall() );
            doGoToPoint(agent, target_point, dist_thr, dash_power, 15.0 );
            break;
        }
        else if(wm.getOpponentNearestToSelf(1,false)->pos().x <= wm.teammate(2)->pos().x ||
                wm.getOpponentNearestToSelf(1,false)->pos().x <= wm.teammate(3)->pos().x){
            target_point.x = (wm.getOpponentNearestToSelf(1,false)->pos().x) - 2;
            target_point.y = wm.getOpponentNearestToSelf(1,false)->pos().y;
            agent->setNeckAction( new rcsc::Neck_TurnToBall() );
            doGoToPoint(agent, target_point, dist_thr, dash_power, 15.0 );
            break;
        }
...
*/
void
RoleSweeper::doBasicMove( PlayerAgent * agent,
...
*/
Vector2D
RoleSweeper::getBasicMoveTarget( PlayerAgent * agent,
                                    const Vector2D & home_pos,
                                    double * dist_thr )
{
    const WorldModel & wm = agent->world();
...
*/
void
RoleSweeper::doDefMidMove( PlayerAgent * agent,
                              const Vector2D & home_pos )
{
...
*/
Vector2D
RoleSweeper::getDefMidMoveTarget( PlayerAgent * agent,
                                     const Vector2D & home_pos )
{
    const WorldModel & wm = agent->world();
...
*/
void
RoleSweeper::doCrossBlockAreaMove( PlayerAgent * agent,
                                      const Vector2D & home_pos )
{
...
*/
void
RoleSweeper::doStopperMove( PlayerAgent * agent,
                               const Vector2D & home_pos )
{
...
*/
void
RoleSweeper::doDangerAreaMove( PlayerAgent * agent,
                                  const Vector2D & home_pos )
{
...
void
RoleSweeper::doMarkMove( PlayerAgent * agent,
                            const Vector2D & home_pos )
{double dash_power = 100.0;
    //-----------------------------------------------
    // tackle
...
*/
bool
RoleSweeper::doDangerGetBall( PlayerAgent * agent,
                                 const Vector2D & home_pos )
{double dash_power = 100.0;
    //////////////////////////////////////////////////////////////////
    // tackle
...
*/
void
RoleSweeper::doGoToPoint( PlayerAgent * agent,
