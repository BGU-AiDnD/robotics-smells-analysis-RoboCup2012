...
int RolePlayer10::S_count_shot_area_hold = 0;

rcsc::Vector2D RolePlayer10::p10AP;

rcsc::Vector2D RolePlayer10::p10DP;

rcsc::Vector2D RolePlayer10::p10VAP;

rcsc::Vector2D RolePlayer10::p10VDP;

rcsc::Vector2D RolePlayer10::p10PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer10::p10PB9 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer10::p10PB11 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer10::p10LB = rcsc::Vector2D(-50.0,-50.0);

//swarm parameters

#define NS 3        //size of neighborhood
#define c1 1.44
#define c2 0.8
...
*/
void
RolePlayer10::execute( rcsc::PlayerAgent * agent )
{
...
void
RolePlayer10::doKick( rcsc::PlayerAgent * agent )
{
...
//      if (wm.self().pos().x >= opp->pos().x ){
      if ((wm.self().pos().x >= opp->pos().x) || (!wm.existKickableTeammate() && countKickableCycleAtSamePoint(agent)<5)){
...
/*      if ( wm.countOpponentsIn(checkSector,3,true) == 0 )
       {
        rcsc::Body_Dribble2008(dribble_target,3.0,
			       rcsc::ServerParam::i().maxDashPower(),5,false
			       ).execute( agent );
	agent->setNeckAction( new rcsc::Neck_TurnToGoalieOrScan() );
	break;
       }
      ...
      else
	{
	   if(countKickableCycleAtSamePoint(agent)<5){
	     ...
	     rcsc::Body_Dribble2008(rcsc::Vector2D(47.0, wm.self().pos().y)
				    ,3.0,rcsc::ServerParam::i().maxPower()
...
*/
void
RolePlayer10::doMove( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p10AP;
        ...
        {
            rcsc::Vector2D target_point = p10DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            //rcsc::Body_GoToPoint(target_point, dist_thr, dash_power, true);
            //rcsc::Body_GoToPointDodge(target_point, dash_power);
...
int
RolePlayer10::countKickableCycleAtSamePoint(rcsc::PlayerAgent * agent){
...
bool
RolePlayer10::moveCentering( rcsc::PlayerAgent * agent )
{
  const rcsc::WorldModel & wm = agent->world();
...
*/
bool
RolePlayer10::doSelfPass( rcsc::PlayerAgent  * agent,
                               const int max_dash_step )
{
    ...
    int dash_step = max_dash_step;
    {
        ...
        for ( int i = 0; i < max_dash_step; ++i )
        {
            ...
            wm.self().playerType().predictStaminaAfterOneDash( rcsc::ServerParam::i(),
                                                               used_power,
                                                               &my_stamina,
                                                               &my_effort,
                                                               &my_recovery );
...
*/
void
RolePlayer10::doMiddleAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer10::doCrossAreaKick( rcsc::PlayerAgent* agent )
{
...
*/
void
RolePlayer10::doShootAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer10::doGoToCrossPoint( rcsc::PlayerAgent * agent,
...
*/
bool
RolePlayer10::doCheckCrossPoint( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
void RolePlayer10::doOffensiveMiddleKick( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer10::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer10::swarmUpdatePosition(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
...
void
RolePlayer10::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(9) != 0) p10PB9 = wm.teammate(9)->pos();

        if(wm.teammate(10) != 0) p10PB = wm.teammate(10)->pos();

        if(wm.teammate(11) != 0) p10PB11 = wm.teammate(11)->pos();

        if(wm.teammate(11) != 0) p10LB = wm.teammate(11)->pos();

        p10VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p10VDP.x = wm.ball().pos().x - wm.self().pos().x;

        p10AP = rcsc::Vector2D(-50.0,0.0);
        p10DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        {
            //X
            if(wm.teammate(i) != 0 && (wm.teammate(i)->pos().x > PB[i-9].x) )
                PB[i-9] = wm.teammate(i)->pos();
        }

        //only updates variable if new position is better and valid

        if(p10PB9 != PB[0]) p10PB9 = PB[0];

        if (p10PB != PB[1]) p10PB = PB[1];

        if (p10PB11 != PB[2]) p10PB11 = PB[2];
        ...
        //speed.x = 0.2*p10VAP.x;
        speed.x += c1 * k * (PB[1].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        speed.y = 0.0;

        p10AP = wm.self().pos() + speed;
        p10DP = wm.self().pos() + speed;

        p10VAP = p10AP;       //represents inertia
        p10VDP = p10DP;
    }
}
...
void
RolePlayer10::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
