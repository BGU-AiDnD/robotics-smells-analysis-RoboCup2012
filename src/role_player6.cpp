...
int RolePlayer6::S_count_shot_area_hold = 0;

//#define USE_BHV_DEFENSIVE_HALF_DANGER_MOVE
#define USE_BHV_DEFENSIVE_HALF_DEFENSIVE_MOVE

rcsc::Vector2D RolePlayer6::p6AP;

rcsc::Vector2D RolePlayer6::p6DP;

rcsc::Vector2D RolePlayer6::p6VAP;

rcsc::Vector2D RolePlayer6::p6VDP;

rcsc::Vector2D RolePlayer6::p6PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6PB5 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6PB7 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6PB8 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6PB9 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6PB10 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer6::p6LB = rcsc::Vector2D(-70.0,-70.0);
...
*/
void
RolePlayer6::execute( rcsc::PlayerAgent* agent )
{
    //////////////////////////////////////////////////////////////
    // play_on play
...
*/
void
RolePlayer6::doKick( rcsc::PlayerAgent* agent )
{
...
*/
void
RolePlayer6::doMove( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p6AP;
        ...
        {
            rcsc::Vector2D target_point = p6DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            //rcsc::Body_GoToPoint(target_point, dist_thr, dash_power, true);
            //rcsc::Body_GoToPointDodge(target_point, dash_power);
...
*/
void
RolePlayer6::doCrossBlockMove( rcsc::PlayerAgent* agent,
...
//     }
    ...
    if ( wm.existKickableTeammate() )
    {
        dash_power = std::min( rcsc::ServerParam::i().maxPower() * 0.5,
                               dash_power );
    }
...
*/
void
RolePlayer6::doOffensiveMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
void
RolePlayer6::doDefensiveMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
void
RolePlayer6::doDribbleBlockMove( rcsc::PlayerAgent* agent,
                                       const rcsc::Vector2D& home_pos )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
    ...
    else
    {
        ...
        dash_power = rcsc::min_max( 0.0, dash_power, rcsc::ServerParam::i().maxPower() );
        rcsc::dlog.addText( rcsc::Logger::ROLE,
                            __FILE__": doDribbleBlockMove() dash_power=%.1f",
                            dash_power );
    }
...
*/
void
RolePlayer6::doCrossAreaMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
    rcsc::dlog.addText( rcsc::Logger::TEAM,
...
*/
void
RolePlayer6::doShootAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
bool
RolePlayer6::doCheckCrossPoint( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer6::doMiddleAreaKick( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
void RolePlayer6::doOffensiveMiddleKick( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer6::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer6::swarmUpdatePosition(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
...
void
RolePlayer6::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(5) != 0) p6PB5 = wm.teammate(5)->pos();

        if(wm.teammate(6) != 0) p6PB = wm.teammate(6)->pos();

        if(wm.teammate(7) != 0) p6PB7 = wm.teammate(7)->pos();

        if(wm.teammate(8) != 0) p6PB8 = wm.teammate(8)->pos();

        if(wm.teammate(9) != 0) p6PB9 = wm.teammate(9)->pos();

        if(wm.teammate(10) != 0) p6PB10 = wm.teammate(10)->pos();

        if(wm.teammate(10) != 0) p6LB = wm.teammate(10)->pos();

        p6VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p6VDP.x = wm.ball().pos().x - wm.self().pos().x;

        p6AP = rcsc::Vector2D(-50.0,0.0);
        p6DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        if(p6PB5 != PB[0]) p6PB5 = PB[0];

        if (p6PB != PB[1]) p6PB = PB[1];

        if (p6PB7 != PB[2]) p6PB7 = PB[2];

        if (p6PB8 != PB[3]) p6PB8 = PB[3];

        if (p6PB9 != PB[4]) p6PB9 = PB[4];

        if (p6PB10 != PB[5]) p6PB10 = PB[5];
        ...
        //speed.x = 0.2*p6VAP.x;
        speed.x += c1 * k * (PB[1].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        speed.y = 0.0;

        p6AP = wm.self().pos() + speed;
        p6DP = wm.self().pos() + speed;

        p6VAP = p6AP;       //represents inertia
        p6VDP = p6DP;
    }
}
...
void
RolePlayer6::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
