...
rcsc::Vector2D RolePlayer2::p2AP;
     
rcsc::Vector2D RolePlayer2::p2DP;

rcsc::Vector2D RolePlayer2::p2VAP;

rcsc::Vector2D RolePlayer2::p2VDP;
     
rcsc::Vector2D RolePlayer2::p2PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer2::p2PB3 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer2::p2PB4 = rcsc::Vector2D(-70.0,-70.0);;

rcsc::Vector2D RolePlayer2::p2PB5 = rcsc::Vector2D(-70.0,-70.0);;

rcsc::Vector2D RolePlayer2::p2PB6 = rcsc::Vector2D(-70.0,-70.0);;
     
rcsc::Vector2D RolePlayer2::p2LB = rcsc::Vector2D(-70.0,-70.0);
...
*/
void
RolePlayer2::execute( PlayerAgent * agent )
{
...
*/
void
RolePlayer2::doKick( PlayerAgent * agent )
{
...
*/
void
RolePlayer2::doMove( PlayerAgent * agent )
{
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p2AP;
        ...
        {
            rcsc::Vector2D target_point = p2DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            agent->setNeckAction( new rcsc::Neck_TurnToBall() );
        }          
...
*/
void
RolePlayer2::doBasicMove( PlayerAgent * agent,
...
*/
Vector2D
RolePlayer2::getBasicMoveTarget( PlayerAgent * agent,
                                    const Vector2D & home_pos,
                                    double * dist_thr )
{
    const WorldModel & wm = agent->world();
...
*/
void
RolePlayer2::doDefMidMove( PlayerAgent * agent,
                              const Vector2D & home_pos )
{
...
*/
Vector2D
RolePlayer2::getDefMidMoveTarget( PlayerAgent * agent,
                                     const Vector2D & home_pos )
{
    const WorldModel & wm = agent->world();
...
*/
void
RolePlayer2::doCrossBlockAreaMove( PlayerAgent * agent,
                                      const Vector2D & home_pos )
{
...
*/
void
RolePlayer2::doStopperMove( PlayerAgent * agent,
                               const Vector2D & home_pos )
{
...
*/
void
RolePlayer2::doDangerAreaMove( PlayerAgent * agent,
                                  const Vector2D & home_pos )
{
...
void
RolePlayer2::doMarkMove( PlayerAgent * agent,
                            const Vector2D & home_pos )
{
    //-----------------------------------------------
    // tackle
...
*/
bool
RolePlayer2::doDangerGetBall( PlayerAgent * agent,
                                 const Vector2D & home_pos )
{
    //////////////////////////////////////////////////////////////////
    // tackle
...
*/
void
RolePlayer2::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer2::swarmUpdatePosition(PlayerAgent * agent)
{    
    const WorldModel & wm = agent->world();
...
void
RolePlayer2::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(2) != 0) p2PB = wm.teammate(2)->pos();

        if(wm.teammate(3) != 0) p2PB3 = wm.teammate(3)->pos();

        if(wm.teammate(4) != 0) p2PB4 = wm.teammate(4)->pos();

        if(wm.teammate(5) != 0) p2PB5 = wm.teammate(5)->pos();

        if(wm.teammate(6) != 0) p2PB6 = wm.teammate(6)->pos();

        if(wm.teammate(6) != 0) p2LB = wm.teammate(6)->pos();
       
        p2VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p2VDP.x = wm.ball().pos().x - wm.self().pos().x;
        
        p2AP = rcsc::Vector2D(-50.0,0.0);
        p2DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        if(p2PB != PB[0]) p2PB = PB[0];
        
        if (p2PB3 != PB[1]) p2PB3 = PB[1];
        
        if (p2PB4 != PB[2]) p2PB4 = PB[2];
        
        if (p2PB5 != PB[3]) p2PB5 = PB[3];
        
        if (p2PB6 != PB[4]) p2PB6 = PB[4];
        ...
        std::cout << std::endl << std::endl << "k: " << k << std::endl;
        std::cout << std::endl << std::endl << "c1: " << c1 << std::endl;
        std::cout << std::endl << std::endl << "c2: " << c2 << std::endl;
        std::cout << std::endl << std::endl << "PB[0].x: " << PB[0].x << std::endl;
        std::cout << std::endl << std::endl << "posX: " << wm.self().pos().x << std::endl;
        std::cout << std::endl << std::endl << "LB.x: " << LB.x << std::endl;
        std::cout << std::endl << std::endl << "d1: " << PB[0].x - wm.self().pos().x << std::endl;
        std::cout << std::endl << std::endl << "d2: " << LB.x - wm.self().pos().x << std::endl;
                
        //speed.x = 0.2*p2VAP.x;
        speed.x += c1 * k * (PB[0].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        std::cout << std::endl << std::endl << "s1: " <<c1 * k * (PB[0].x - wm.self().pos().x) << std::endl;
        std::cout << std::endl << std::endl << "s2: " << c2 * k * (LB.x - wm.self().pos().x) << std::endl;

        speed.y = 0.0;
            
        p2AP = wm.self().pos() + speed;
        p2DP = wm.self().pos() + speed;
        std::cout << std::endl << std::endl << "destiny: " << p2AP.x << std::endl;
        std::cout << std::endl << std::endl << "pos: " << wm.self().pos().x << std::endl;
        p2VAP = p2AP;       //represents inertia
        p2VDP = p2DP;
    }
}
/*
...
void
RolePlayer2::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
