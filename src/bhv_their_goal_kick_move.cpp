...
Bhv_TheirGoalKickMove::execute( rcsc::PlayerAgent * agent )
{
    static const rcsc::Rect2D
        ...
        ( rcsc::Vector2D( rcsc::ServerParam::i().theirPenaltyAreaLineX() - 0.75,
    ...
    double dash_power
        = wm.self().getSafetyDashPower( rcsc::ServerParam::i().maxPower() * 0.8 );
...
Bhv_TheirGoalKickMove::doNormal( rcsc::PlayerAgent * agent )
{
    ...
    rcsc::dlog.addText( rcsc::Logger::TEAM,
                        __FILE__": doNormal move_pos(%.1f %,1f)",
...
Bhv_TheirGoalKickMove::doChaseBall( rcsc::PlayerAgent * agent )
{
    ...
    const rcsc::Rect2D their_penalty
        ( rcsc::Vector2D( pen_x, -pen_y ),
