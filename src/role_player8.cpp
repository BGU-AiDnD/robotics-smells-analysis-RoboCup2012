...
int RolePlayer8::S_count_shot_area_hold = 0;

#define USE_BHV_OFFENSIVE_HALF_DEFENSIVE_MOVE
#define USE_BHV_OFFENSIVE_HALF_OFFENSIVE_MOVE

rcsc::Vector2D RolePlayer8::p8AP;

rcsc::Vector2D RolePlayer8::p8DP;

rcsc::Vector2D RolePlayer8::p8VAP;

rcsc::Vector2D RolePlayer8::p8VDP;

rcsc::Vector2D RolePlayer8::p8PB = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8PB5 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8PB6 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8PB7 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8PB9 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8PB10 = rcsc::Vector2D(-70.0,-70.0);

rcsc::Vector2D RolePlayer8::p8LB = rcsc::Vector2D(-70.0,-70.0);
...
*/
void
RolePlayer8::execute( rcsc::PlayerAgent * agent )
{
    //////////////////////////////////////////////////////////////
    // play_on play
...
*/
void
RolePlayer8::doKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer8::doMove( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
    ...
    case Strategy::BA_ShootChance:
        ...
        {
            rcsc::Vector2D target_point = p8AP;
        ...
        {
            rcsc::Vector2D target_point = p8DP;
            double dash_power = Strategy::get_set_play_dash_power( wm );
            doGoToPoint( agent, target_point, dist_thr, dash_power, 15.0 );
            //rcsc::Body_GoToPoint(target_point, dist_thr, dash_power, true);
            //rcsc::Body_GoToPointDodge(target_point, dash_power);
...
*/
void
RolePlayer8::doDribbleAttackKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer8::doShootChanceKick( rcsc::PlayerAgent * agent )
{
...
*/
void
RolePlayer8::doOffensiveMove( rcsc::PlayerAgent * agent,
...
*/
void
RolePlayer8::doDefensiveMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
void
RolePlayer8::doShootAreaMove( rcsc::PlayerAgent * agent,
                                    const rcsc::Vector2D & home_pos )
{
...
*/
bool
RolePlayer8::doShootAreaIntercept( rcsc::PlayerAgent * agent,
                                         const rcsc::Vector2D & /*home_pos*/ )
{
    const rcsc::WorldModel & wm = agent->world();
    const int self_min = wm.interceptTable()->selfReachCycle();
...
int
RolePlayer8::countKickableCycleAtSamePoint(rcsc::PlayerAgent * agent){
...
*/
void
RolePlayer8::doShootAreaKick( rcsc::PlayerAgent* agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
bool
RolePlayer8::doCheckCrossPoint( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
void RolePlayer8::doOffensiveMiddleKick( PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer8::doCrossAreaKick( rcsc::PlayerAgent* agent )
{
...
*/
void
RolePlayer8::doDribbleAttackAreaKick( rcsc::PlayerAgent* agent )
{
    // !!!check stamina!!!
    static bool S_recover_mode = false;
...
*/
bool
RolePlayer8::doStraightDribble( rcsc::PlayerAgent * agent )
{
    const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer8::doSideForwardDribble( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
rcsc::Vector2D
RolePlayer8::getDribbleTarget( rcsc::PlayerAgent * agent )
{
...
*/
int
RolePlayer8::getDribbleAttackDashStep( const rcsc::PlayerAgent * agent,
...
bool
RolePlayer8::kickCentering( rcsc::PlayerAgent * agent )
{
  //センタリング 090704
  const rcsc::WorldModel & wm = agent->world();
...
*/
void
RolePlayer8::doMiddleAreaKick( rcsc::PlayerAgent * agent )
{
    rcsc::dlog.addText( rcsc::Logger::ROLE,
...
*/
void
RolePlayer8::doGoToPoint( PlayerAgent * agent,
...
//#if 0
void
RolePlayer8::swarmUpdatePosition(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
...
void
RolePlayer8::swarmAttack(PlayerAgent * agent)
{
    const WorldModel & wm = agent->world();
    ...
    {
        if(wm.teammate(5) != 0) p8PB5 = wm.teammate(5)->pos();

        if(wm.teammate(6) != 0) p8PB6 = wm.teammate(6)->pos();

        if(wm.teammate(7) != 0) p8PB7 = wm.teammate(7)->pos();

        if(wm.teammate(8) != 0) p8PB = wm.teammate(8)->pos();

        if(wm.teammate(9) != 0) p8PB9 = wm.teammate(9)->pos();

        if(wm.teammate(10) != 0) p8PB10 = wm.teammate(10)->pos();

        if(wm.teammate(10) != 0) p8LB = wm.teammate(10)->pos();

        p8VAP.x = wm.ball().pos().x - wm.self().pos().x;
        p8VDP.x = wm.ball().pos().x - wm.self().pos().x;

        p8AP = rcsc::Vector2D(-50.0,0.0);
        p8DP = rcsc::Vector2D(-50.0,0.0);
    ...
    else
    {
        ...
        if(p8PB5 != PB[0]) p8PB5 = PB[0];

        if (p8PB6 != PB[1]) p8PB6 = PB[1];

        if (p8PB7 != PB[2]) p8PB7 = PB[2];

        if (p8PB != PB[3]) p8PB = PB[3];

        if (p8PB9 != PB[4]) p8PB9 = PB[4];

        if (p8PB10 != PB[5]) p8PB10 = PB[5];
        ...
        //speed.x = 0.2*p8VAP.x;
        speed.x += c1 * k * (PB[3].x - wm.self().pos().x);
        speed.x += c2 * k * (LB.x - wm.self().pos().x);

        speed.y = 0.0;

        p8AP = wm.self().pos() + speed;
        p8DP = wm.self().pos() + speed;

        p8VAP = p8AP;       //represents inertia
        p8VDP = p8DP;
    }
}
...
void
RolePlayer8::swarmDefense(PlayerAgent * agent)
{
    Swarm s;
}
//#endif
