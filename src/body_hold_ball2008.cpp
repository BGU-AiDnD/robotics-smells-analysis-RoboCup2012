                                     ...
                                     std::vector< KeepPoint > & candidates )
{
    ...
    for ( double d = -180.0; d < 180.0; d += dir_step )
    {
        ...
        if ( near_pos.absX() < param.pitchHalfLength() - 0.2
             && near_pos.absY() < param.pitchHalfWidth() - 0.2 )
        {
            ...
            {
                double near_krate = wm.self().playerType().kickRate( near_dist,
                                                                     dir_diff );
                // can stop the ball by 1 step kick
                if ( ball_move.r() * param.ballDecay()
                     < param.maxPower() * near_krate )
                {
#ifdef DEBUG
                    dlog.addText( Logger::ACTION,
        ...
        if ( mid_pos.absX() < param.pitchHalfLength() - 0.2
             && mid_pos.absY() < param.pitchHalfWidth() - 0.2 )
        {
            ...
            if ( kick_power < param.maxPower() )
            {
                ...
                if ( 1 )
                {
                    double mid_krate = wm.self().playerType().kickRate( mid_dist,
                                                                        dir_diff );
                    // can stop the ball by 1 step kick
        ...
        if ( far_pos.absX() < param.pitchHalfLength() - 0.2
             && far_pos.absY() < param.pitchHalfWidth() - 0.2 )
        {
            ...
            if ( kick_power < param.maxPower() )
            {
                ...
                if ( 1 )
                {
                    double far_krate = wm.self().playerType().kickRate( far_dist,
                                                                        dir_diff );
                    // can stop the ball by 1 step kick
                    if ( move_dist * param.ballDecay()
                                      ...
                                      const Vector2D & keep_point )
{
          ...
          ++o )
    {
        ...
        //
        {
            const double max_accel = ( param.maxPower()
...
Body_HoldBall2008::keepFront( PlayerAgent * agent )
{
    ...
    if ( front_pos.absX() > param.pitchHalfLength() - 0.2
         || front_pos.absY() > param.pitchHalfWidth() - 0.2 )
    {
        dlog.addText( Logger::ACTION,
...
Body_HoldBall2008::keepReverse( PlayerAgent * agent )
{
    ...
    for ( ; keep_dist > min_dist; keep_dist -= 0.05 )
    {
        ...
        if ( keep_pos.absX() > param.pitchHalfLength() - 0.2
             || keep_pos.absY() > param.pitchHalfWidth() - 0.2 )
        {
            continue;
        }
        ...
        double new_krate = wm.self().playerType().kickRate( keep_dist,
                                                            dir_diff );
...
Body_HoldBall2008::turnToPoint( PlayerAgent * agent )
{
    ...
    if ( ball_next.absX() > param.pitchHalfLength() - 0.2
         || ball_next.absY() > param.pitchHalfWidth() - 0.2 )
    {
        dlog.addText( Logger::ACTION,
